...
def binary_operator(q_i, q_j):
    ...
    A_j, b_j, r_j = q_j
    del r_j
    return np.where(r_i, A_j, A_j * A_i), A_j * b_i + b_j


def apply_ssm(Lambda_bar, B_bar, C_tilde, input_sequence, reset_sequence, conj_sym):
    ...
    _, xs = jax.lax.associative_scan(
        binary_operator, (Lambda_elements, Bu_elements, reset_sequence)
    )
...
def apply_ssm_with_feedthrough(
    input_sequence, reset_sequence, Lambda_bar, B_bar, C_tilde, D, conj_sym
):
    ys = apply_ssm(Lambda_bar, B_bar, C_tilde, input_sequence, reset_sequence, conj_sym)
...
class S5SSM(nn.Module):
    ...
    def step(self, prev_state, u):
        Lambda_bar, B_bar, C_tilde, u = promote_dtype(
            self.Lambda_bar, self.B_bar, self.C_tilde, u, dtype=self.dtype
        )
        next_state, x = binary_operator(prev_state, (Lambda_bar, B_bar @ u))
        conj_factor = 2 if self.conj_sym else 1
        return next_state, conj_factor * (C_tilde @ x).real

    def __call__(self, input_sequence, reset_sequence, training=True):
        """
        ...
        input_sequence, Lambda_bar, B_bar, C_tilde, D = promote_dtype(
            input_sequence,
            self.Lambda_bar,
            self.B_bar,
            self.C_tilde,
        ...
        ys = jax.vmap(
            apply_ssm_with_feedthrough, in_axes=(0, 0, None, None, None, None, None)
        )(
            input_sequence, reset_sequence, Lambda_bar, B_bar, C_tilde, D, conj_sym
        )  # ys is bsz, L, H
...
class S5Operator(nn.Module):
    ...
    filter_cls: Literal["hyena_S5"] = "hyena_S5"
    post_order_ffn: bool = False
    jit_filter: bool = False
    short_filter_order: int | None = 3
    ...
    def setup(self):
        ...
        ), f"Model dimension {self.d_model} must be divisible by num heads {self.num_heads}"
    ...
    def setup_projections(self, fused_bias_fc, inner_factor, initializer_range=0.02):
        ...
        if not fused_bias_fc:
            linear_cls = partial(nn.Dense, dtype=self.dtype)
    ...
    def setup_filters(self, filter_cls, filter_args):
        ...
        if self.short_filter_order is None:
            self.short_filter = lambda x: x
        else:
        ...
        if self.filter_cls == "hyena_S5":
            ...
            self.filter_fn = [
                init_S5SSM(self.d_model, self.ssm_size, self.ssm_blocks, filter_args)
    ...
    # TODO: give this module an nn.compactectomy
    # TODO: create autoregressive step fn
    @nn.compact
    def __call__(self, u, r, training):
